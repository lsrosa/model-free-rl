...
env = gym.make('Reacher-v4').unwrapped
...
def get_state(state):
    ...
    state = torch.from_numpy(state).unsqueeze(0).type(FloatTensor)
    return state
...
def optimize_model():
    ...
    msb = policy_net(state_batch)
    ab = action_batch
    print(action_batch)
    print(msb, msb.shape)
    state_action_values = msb.gather(1, ab)
    exit()
...
for i_episode in range(num_episodes):
    ...
    for t in range(num_steps_per_epi):
        ...
        state, reward, done, _, info = env.step(jointVel)   # desired joint velocity
